...
class BatchOpenAISemanticSearchService:
    ...
    def batch_call_embeddings(texts, chunk_size=1000):
        ...
        for i in range(0, len(texts), chunk_size):
            ...
            for token in chunk:
                ...
                if total_count + token_count > 13000:
                    break
                limited_chunk.append(token)
            print(limited_chunk)
            print(len(chunk))
            #print(type(chunk))
            #print(chunk)
            try:# Limit the chunk to the maximum allowed tokens
                print("try")
                response = openai.Embedding.create(
                    input=limited_chunk, engine=BASE_MODEL
                )
            except:
                print("except")
                print(f"An error occurred: {e}")
                embeddings_size = len(response["data"][0]["embedding"])
                embeddings += [np.zeros(embeddings_size)]
                # 在这里添加适当的错误处理逻辑
            print("Embedding size:", len(response["data"][0]["embedding"]))
            print("Embedding shape:", np.shape(response["data"][0]["embedding"]))
            #print("Embedding content:", response["data"][0]["embedding"])
    ...
    def search_related_source(self, text_df: pd.DataFrame, target_text):
        ...
        print(f"len(text_df['embedding'].iloc[0]):{len(text_df['embedding'].iloc[0])}")
        print(f"embedding_before:{len(embedding)}")
        if len(embedding) != len(text_df['embedding'].iloc[0]):
            embedding = np.reshape(embedding, (1, -1))
            print(f"embedding_after:{len(embedding)}")
        #
        #min_len = min(len(text_df.index), len(embedding))
        #text_df['similarities'] = text_df['embedding'].apply(
        #    lambda x: cosine_similarity(x[:min_len], embedding[:min_len]))
        text_df['similarities'] = text_df['embedding'].apply(lambda x: cosine_similarity(x, embedding))
        #result_df = text_df.sort_values('similarities', ascending=False).iloc[n*20-1:(n+1)*20-1]
        # Use all text
        result_df = text_df.sort_values('similarities', ascending=False)
        #print(text_df['similarities'].head(5))
        result_df['rank'] = range(1, len(result_df) + 1)
        result_df['docno'] = range(1, len(result_df) + 1)
        #logger.info(f"return page:{n}")
        return result_df

    def search_related_source_sr(self, text_df: pd.DataFrame, target_text, n=1):
        if not self.config.get('source_service').get('is_use_source'):
            col = ['name', 'url', 'url_id', 'snippet', 'text', 'similarities', 'rank', 'docno']
            return pd.DataFrame(columns=col)
        ...
        if self.sender is not None:
            self.sender.send_message(msg_type=MSG_TYPE_SEARCH_STEP, msg="Searching from extracted text")
        target_text += "We rate this claim as true(false)."
        print(f'search_similar() text: {target_text}')
        embedding = BatchOpenAISemanticSearchService.batch_call_embeddings([target_text])[0]
        text_df = BatchOpenAISemanticSearchService.compute_embeddings_for_text_df(text_df)
        print(f"len(text_df['embedding'].iloc[0]):{len(text_df['embedding'].iloc[0])}")
        ...
        #    lambda x: cosine_similarity(x[:min_len], embedding[:min_len]))
        text_df['similarities'] = text_df['embedding'].apply(lambda x: cosine_similarity(x, embedding))
        result_df = text_df.sort_values('similarities', ascending=False).iloc[n*30-1:(n+1)*30-1]
        # Use all text
        #result_df = text_df.sort_values('similarities', ascending=False)
        print(text_df['similarities'].head(5))
        result_df['rank'] = range(1, len(result_df) + 1)
